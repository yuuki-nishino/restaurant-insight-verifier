...
```mermaid
flowchart TD
    A[口コミ投入<br/>テキスト貼付/CSV] --> B[Jev API<br/>1件ずつ構造化分類]
    B --> C[集計処理<br/>アスペクト別/メニュー別<br/>（サーバーのメモリ上で計算）]
...
| 画面                 | 内容                                                                                         |
| -------------------- | -------------------------------------------------------------------------------------------- |
| ① 口コミ投入         | テキストエリアへの一括貼付 or CSVアップロード、「分析開始」ボタン（DBには保存せずそのままサーバーに送信して分析処理を開始する） |
