...
| ③ ダッシュボード     | Jev APIでの処理時間（合計・1件あたり平均、秒単位）、アスペクト別件数（味/接客/待ち時間/清潔さ/コスパ/見た目/ニオイ）、感情の内訳（ポジティブ/ネガティブ/ニュートラル/賛否混在）、対応ガイド（お礼/謝罪推奨、オペレーション/メニュー・レシピ改善）の件数。口コミ別カードでは全項目のパーセンテージを表示（Jevによる分類であることを明示） |
